  ...
  export default {
    ...
    methods: {
      onSelect(rowIndex, selectedData) {
        ...
        vm.product = vm1._id; //Assign as foreign key
      ...
      filterFn (val, update) {
        if (val === '') {
          update(() => {
            this.options = this.data_product.map((f)=> f.label)
          })
          return
        }
        update(() => {
          const needle = val.toLowerCase()
          this.options = this.data_product.map((f)=> f.label).filter(v => v.toLowerCase().indexOf(needle) > -1)
        })
      },
      addDataDetail() {
      ...
      async onSubmit() {
        ...
        vm_data.supplier = vm_data.supplier_obj._id; //get id of supplier selected
        ...
        await self.$store.dispatch('stock/storeStock', vm_data).then(async function (data) {
          if (data.status) {
